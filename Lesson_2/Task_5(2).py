...
new_el = int(input('Введите новое число рейтинга: '))
el_of_list = my_list.count(new_el)
for el in my_list:
    if el_of_list > 0:
        index = my_list.index(new_el)
        my_list.insert(index + el_of_list, new_el)
        break
    else:
        if new_el > el:
            el_index = my_list.index(el)
            my_list.insert(el_index, new_el)
            break
